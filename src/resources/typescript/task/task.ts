...
const hasContactPerson = (
  document.querySelector("meta[name=userHasContactPerson]") as HTMLMetaElement
).content;
const table = new DataTable("#tasks-table", {
  ...
  columns: [
    ...
    },
    {
      orderable: false,
      data: (row) =>
        elements.priority(row.isPriority, row.id, strToBool(hasContactPerson)),
      name: "priority",
    },
    {
...
const inputs = {
  ...
  category: <HTMLSelectElement>(
    (modalInput as Array<HTMLSelectElement | HTMLInputElement>)[1]
  ),
  ...
  priority: <HTMLInputElement>(
    (modalInput as Array<HTMLSelectElement | HTMLInputElement>)[4]
  ),
};
...
modal.submitButton?.addEventListener("click", (e) => {
  ...
  if (!isEditMode) {
    ...
    put(
      `/tasks/${sessionStorage.getItem("editId")}`,
      {
        name: inputs.name.value,
        category: inputs.category.value,
        due_date: inputs.dueDate.value,
...
document.querySelector("#tasks-table")?.addEventListener("click", (e) => {
  ...
  const setPriorityBtn = dispatcher.closest(
    "#set-priority"
  ) as HTMLInputElement;
  ...
  if (delBtn) {
    ...
    get(`tasks/${viewBtn.dataset.id}`)
      ...
      .then((task) => {
        ...
        (
          noteModal.element.querySelector("p") as HTMLParagraphElement
        ).textContent = task.note;
        noteModal.open();
      });
  } else if (setPriorityBtn) {
    put(`/tasks/priority/set/${setPriorityBtn.dataset.id}`, {
      priority: setPriorityBtn.checked,
    }).then((res) => {
      if (!res.ok) {
